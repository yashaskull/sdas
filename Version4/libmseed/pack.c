...
//#include "libdigitizer.h"
//#include "libprocessing.h"
//#include "/home/arvid/Desktop/FINAL/Version3/src/msrecord.h"
#include "libmseed.h"
#include "packdata.h"
#include "libdali.h"
...
void record_handler(char *record, int reclen, void *ofp, FILE *fp_log)
{
	...
	if ( fwrite(record, reclen, 1, (FILE *)ofp) != 1 )
  {
  	...
  	fprintf(fp_log, "Error writing record to output file.\n");
  	fflush(fp_log);
  }
}
...
///* Add Blank 100 Blockette for storing floating point sample rates */////////
 	if (!msr->Blkt100)
  {
    ...
    if (verbose > 2)
      ms_log (1, "%s: Adding 100 Blockette\n", srcname);

    if (!msr_addblockette (msr, (char *)&blkt100, sizeof (struct blkt_100_s), 100, 0))
    {
      ms_log (2, "msr_pack(%s): Error adding 1000 Blockette\n", srcname);
      free (rawrec);
      return -1;
...
///////////////////////////////////////////////////////////////////////////////
  headerlen = msr_pack_header_raw (msr, rawrec, msr->reclen, headerswapflag, 1,
	...
	//printf("returned from msr_pack_header_raw\n");
  if (headerlen == -1)
  {
...
msr_pack_header_raw (MSRecord *msr, char *rawrec, int maxheaderlen,
                     ...
                     char *srcname, flag verbose)
{
	//printf("inside msr_pack_header_raw\n");
  ...
  {
  	//printf("Traversing blockette chain \n");
  	//printf("%d\n", cur_blkt->blkt_type);
