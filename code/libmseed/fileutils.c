...
{
	printf("Inside ms_record_handler_int\n");
	//printf("%f\n", msr->samplerate);
...
msr_writemseed (MSRecord *msr, /*const char *msfile,*/ flag overwrite,
                int reclen, flag encoding, flag byteorder, flag verbose , char **record /*char **record*/)
{
			printf("a\n");
  char srcname[50];
  int packedrecords = 0;
	...
	*record = (char *)malloc (reclen);
	...
	if (*record == NULL)
	{
		printf("Error: Could not allocate memory for mseed record\n");
		return -1;
	}
  /* Pack the MSRecord */
  ...
  if (msr->numsamples > 0)
  {
    ...
    msr->byteorder = byteorder;
		/* Remember to DELETE!!!! */
		printf("Hererer\n");
   // packedrecords = msr_pack (msr, NULL, /*ofp,*/ 1, verbose - 1, &record_temp);
   
		if (record_temp != NULL)
		{
			memcpy(*record, record_temp, msr->reclen);
    	free(record_temp);
    }
 ...
 ***************************************************************************/
int
msr_writemseed_file (MSRecord *msr, const char *msfile, flag overwrite,
                int reclen, flag encoding, flag byteorder, flag verbose, char **record /*char **record*/)
{
	printf("inside msr write\n");
  FILE *ofp;
  char srcname[50];
  ...
  if (msr->numsamples > 0)
  {
   ...
   // packedrecords = msr_pack (msr, &ms_record_handler_int, /*ofp,*/ NULL, 1, verbose - 1, &record_temp);
		//printf("return from msr_pack\n");
		//printf("%s\n", record_temp);
		memcpy(*record, record_temp, msr->reclen);
    free(record_temp);
    if (packedrecords < 0)
                ...
                int reclen, flag encoding, flag byteorder, flag verbose)
{
	...
	printf("Insideee mst_write mseed\n");
  FILE *ofp;
  char srcname[50];
  char *perms       = (overwrite) ? "wb" : "ab";
  int packedrecords = 0;
